...
import { useTradingProStore, type Drawing } from './store/store';
import { useDataService } from './hooks/useDataService';
import { useReplayEngine } from './hooks/useReplayEngine';
import { useDrawingService, type InProgressDrawing } from './hooks/useDrawingService';
import type { LogicalRange, CandlestickData, UTCTimestamp, WhitespaceData, MouseEventParams } from 'lightweight-charts';
...
// Your function to determine if the timeframe is daily or higher
...
function App() {
    ...
    const dataForChart = useMemo((): CandlestickData<UTCTimestamp>[] => {
        ...
        const offsetInSeconds = isDailyOrHigher ? 12 * 60 * 60 : 60 * 60;

        let dataToProcess: CandlestickData<UTCTimestamp>[];

        if (isReplayMode && replayData.length > 0) {
            dataToProcess = replayData.slice(0, replayCurrentIndex + 1).map(d => ({
                ...d,
                time: ((d.time as number) + offsetInSeconds) as UTCTimestamp,
            }));
        } else {
            const sortedLiveData = [...liveData].sort((a, b) => (a.time as number) - (b.time as number));
            if (sortedLiveData.length === 0) {
                return [];
            }
            dataToProcess = sortedLiveData.map(d => ({
                ...d,
                time: ((d.time as number) + offsetInSeconds) as UTCTimestamp,
            }));
        }
    ...
    const handleVisibleLogicalRangeChange = useCallback((range: LogicalRange | null) => {
        ...
        const lastCandleIndex = dataForChart.length -1;
    ...
    const onChartClickHandler = useCallback((param: MouseEventParams) => {
        if (replayState === 'arming' && param.time) {
            replayEngine.selectReplayStart(param.time as UTCTimestamp);
        } else {
            handleChartClick(param);
        }
    }, [replayState, replayEngine, handleChartClick]);

    const onCrosshairMoveHandler = useCallback((param: MouseEventParams) => {
    ...
    return (
        <div className='bg-gray-900 w-screen h-screen flex flex-col text-white'>
            ...
            <header className='flex-shrink-0 flex items-center justify-between h-10 px-2 border-b-2 border-[#2D2D2D] bg-[#0e0e0e]'>
            ...
            <div className='flex flex-col md:flex-row flex-1 overflow-hidden'>
                ...
                <main className='flex-1 relative'>
                    ...
                    <ChartComponent
                        ...
                        onCrosshairMove={onCrosshairMoveHandler}
                        ...
                        initDrawingService={initDrawingService}
                        inProgressDrawing={inProgressDrawing}
                        drawings={drawings}
                        hoveredDrawing={hoveredDrawing}
                        timezone={timezone}
                    />
                    {showScrollButton && (
